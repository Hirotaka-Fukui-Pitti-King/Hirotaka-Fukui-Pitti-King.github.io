...
返り値は、**（1）$z$ のグリッド**、**（2）遷移確率行列**、**（3）定常分布** です [^7]。
...
[^7]: 単純に自分の持っているTauchen法のコード(定量的マクロ経済学と数値計算という本についてきたコードを修正したもの)を使っているだけなので、皆さんがやるなら`QuantEcon.jl` の `tauchen` 関数を使ったほうが楽です。
...
- `searchsortedlast` は **単調グリッド前提**。`k_grid` は増加等間隔が望ましい。  
- 大規模格子では $P$ は **疎行列** になるため、`SparseMatrixCSC` で構築し、  
  `IterativeSolvers.jl` などの反復ソルバを使うとメモリと速度が改善する。
